...
# YOLO 모델을 이용하여 객체 탐지
def yolo_detect(img, yolo_model, output_layers):
    # 이미지 크기 정보 확인
    ...
    for output in output3:
        for vec85 in output:
            ...
            # 동물 class만 추출 - 14(bird), 15(cat), 16(dog), 17(horse), 18(sheep), 19(cow), 20(elephant), 21(bear), 22(zebra), 23(giraffe)
...
model, output_layers, class_names = construct_yolo_v3()             # YOLO 모델 생성
colors = np.random.uniform(0, 255, size=(len(class_names), 3))      # 클래스별 색상 랜덤 생성

img = cv.imread("imgs/rpi_img3.jpg")
if img is None: sys.exit("Could not read the image.")

res = yolo_detect(img, model, output_layers)
...
cv.putText(img, text, (margin, margin), cv.FONT_HERSHEY_PLAIN, 2, (0, 255, 0), 2)
